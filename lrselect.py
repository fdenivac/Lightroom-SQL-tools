...
def main():
    ...
    parser.add_argument('-z', '--filesize', action='store_true', help='Compute and display files size selection. Alternative: add a column "filesize"')
    parser.add_argument('-n', '--max-lines', type=int, default=-1, help='Max number of results to display (-1 means all results)')
    parser.add_argument('-f', '--file', help='UUIDs photos file : replace the criteria parameter which is ignored')
    ...
    if args.sql:
        print(' * SQL request = ', lrobj.select_generic(','.join(columns_lr), args.criteria, sql=True))
    ...
    if args.file:
        ...
        try:
            ...
            print(f' ==> Failed to open input file "{_e.filename}"', file=sys.stderr)
            return
        # build rows ...
        bad_uuids = []
        rows = []
        for uuid in uuids:
            try:
                row = lrobj.select_generic(','.join(columns_lr), f'uuid="{uuid}"').fetchone()
                if row is None:
                ...
                return
        if bad_uuids:
            print(f"=> WARNING found uuids invalid ({len(bad_uuids)}/{len(uuids)}) : {', '.join(bad_uuids)}")
    else:
        try:
            rows = lrobj.select_generic(','.join(columns_lr), args.criteria).fetchall()
        except (LRSelectException)  as _e:
